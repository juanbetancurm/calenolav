...
When Google integration is configured, the production API starts one bounded reconciliation runner after the HTTP listener is ready. It executes immediately and on a validated interval, suppresses overlapping batches, emits aggregate-only logs, and waits for active work during graceful shutdown. Deployments without Google configuration do not create the runner.

Calendar notification persistence uses an opaque watch-channel registry tied to the encrypted tenant connection. Only a SHA-256 channel-token digest, Google resource identifier, expiration, monotonic message number, resource state, and receipt time are stored. One atomic update ignores unknown, mismatched, expired, duplicate, and reordered notifications without reading or persisting calendar event content.

With complete Google configuration, `POST /google/calendar/notifications` accepts only strict Google notification headers and no body. Accepted and ignored messages share one empty privacy-safe 204 response, while validation and dependency failures expose stable errors only. Channel registration and renewal remain the next synchronization boundary.
