...
An authenticated tenant owner can start authorization with `POST /tenants/:tenantId/google/oauth/start`. The backend creates a ten-minute encrypted attempt and returns a `303` redirect to Google with offline access, S256 PKCE, and the minimum free/busy plus owned-event scopes. The route is absent when OAuth configuration is not supplied; callback and token exchange are the next Step 5 unit.
